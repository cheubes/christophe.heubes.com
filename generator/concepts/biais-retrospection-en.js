...
module.exports = {
  id:          'biais-retrospection-en',
  title:       'Hindsight Bias',
  category:    'Biases & Heuristics',
  tagline:     '"I knew it all along."',
  description: "Bias documented by Baruch Fischhoff (1975): after learning the outcome of an event, people overestimate the probability they would have assigned to it beforehand — memory rewrites itself.",
  ...
  render(ctx) {
    ...
    <text x="600" y="${timeY + 38}" text-anchor="middle"
          ...
          letter-spacing="2" fill="${p.gold}" opacity="0.65">EVENT</text>`;
    ...
    <text x="${rx + panelW / 2}" y="${panelY + 36}" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="500" font-size="13"
          letter-spacing="2" fill="${p.gold}" opacity="0.70">AFTER</text>
    <text x="${rx + panelW / 2}" y="${panelY + 56}" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="400" font-size="11"
          fill="${p.gold}" opacity="0.45">reconstructed memory</text>
    ...
    <text x="${rx + panelW / 2}" y="${panelY + 298}" text-anchor="middle"
          ...
          letter-spacing="1" fill="${p.gold}" opacity="0.65">"I knew it"</text>
    <text x="${rx + panelW / 2}" y="${panelY + 328}" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="400" font-size="12"
          fill="${p.gold}" opacity="0.40">retrospective certainty</text>`;
    ...
    <text x="${(lx + panelW + rx) / 2}" y="${midY - 12}" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="400" font-size="11"
          letter-spacing="1" fill="${p.muted}" opacity="0.28">memory rewrite</text>`;

    const caption = `
    <text x="600" y="858" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="400" font-size="13"
          letter-spacing="1.5" fill="${p.muted}" opacity="0.32">Memory rewrites itself in light of the known outcome.</text>`;
