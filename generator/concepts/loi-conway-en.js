module.exports = {
  ...
  render(ctx) {
    ...
    // ── Node positions (same triangle topology on both sides) ─────────────
    const L = {
      top: { x: 300, y: 345 },
      bl:  { x: 185, y: 545 },
      br:  { x: 415, y: 545 },
    ...
    //   Identical topology on both panels (complete triangle)
    ...
    // ── Left nodes: circles (human teams) ────────────────────────────────
    const rL = 36;
    ...
    // ── Right nodes: rounded rectangles (system components) ───────────────
    const rw = 84, rh = 38;
    ...
    const rightNodes = rectNode(R.top, 'PORTAL')
                     + rectNode(R.bl,  'WEB')
                     + rectNode(R.br,  'API');

    // ── Contextual labels below each panel ────────────────────────────────
    const botLabels = `
    <text x="300" y="634" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="400" font-size="14"
          letter-spacing="1" fill="${p.muted}" opacity="0.50">team communication</text>
    <text x="900" y="634" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="400" font-size="14"
          letter-spacing="1" fill="${p.muted}" opacity="0.50">system interfaces</text>`;
    ...
    <text x="600" y="453" text-anchor="middle"
          ...
          letter-spacing="2" fill="${p.muted}" opacity="0.45">MIRRORS</text>`;
