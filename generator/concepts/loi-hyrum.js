...
module.exports = {
  ...
  render(ctx) {
    ...
    <text x="420" y="192" text-anchor="middle"
          ...
          letter-spacing="2" fill="${p.muted}" opacity="0.38">PEU D'UTILISATEURS</text>
    ...
    <text x="${apiX + apiW / 2}" y="${topApiY + 36}" text-anchor="middle"
          ...
          letter-spacing="1.5" fill="${p.gold}" opacity="0.72">INTERFACE PRÉVUE</text>
    <text x="${apiX + apiW / 2}" y="${topApiY + 58}" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="400" font-size="11"
          fill="${p.gold}" opacity="0.45">comportements documentés</text>
    ...
    <text x="${apiX + apiW / 2}" y="${topApiY + secH + 38}" text-anchor="middle"
          ...
          letter-spacing="1" fill="${p.muted}" opacity="0.25">IMPLÉMENTATION</text>
    <text x="${apiX + apiW / 2}" y="${topApiY + secH + 58}" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="400" font-size="11"
          fill="${p.muted}" opacity="0.16">(opaque)</text>`;

    // 3 user nodes → connected to interface only
    ...
    // --- BOTTOM PANEL: MANY USERS ---
    const botApiY = 518, botSecH = 78;
    const botSections = [
      { label: 'INTERFACE PRÉVUE',    sub: 'comportements documentés', col: p.gold,  op: 0.65, fillOp: 0.08, dashes: '' },
      { label: 'EDGE CASES',          sub: 'comportements limites',    col: p.muted, op: 0.40, fillOp: 0.04, dashes: 'stroke-dasharray="5 3"' },
      { label: 'BUGS / QUIRKS',       sub: 'comportements anormaux',   col: p.muted, op: 0.28, fillOp: 0.03, dashes: 'stroke-dasharray="4 4"' },
      { label: 'IMPLICITE',           sub: 'non documenté',            col: p.muted, op: 0.18, fillOp: 0.02, dashes: 'stroke-dasharray="3 5"' },
    ];

    let botApi = `
    <text x="420" y="490" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-weight="500" font-size="13"
          letter-spacing="2" fill="${p.gold}" opacity="0.55">NOMBREUX UTILISATEURS</text>`;
    ...
    // 8 user nodes, all connected to all sections
    const botNodes = [
    ...
    <text x="600" y="925" text-anchor="middle"
          ...
          letter-spacing="1.5" fill="${p.muted}" opacity="0.32">Plus il y a d'utilisateurs, plus les comportements imprévus deviennent des contrats.</text>`;
