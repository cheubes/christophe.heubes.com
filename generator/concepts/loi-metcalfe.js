...
module.exports = {
  id:          'loi-metcalfe',
  title:       'Loi de Metcalfe',
  category:    'Ingénierie & Technologie',
  tagline:     "La valeur d'un réseau croît comme le carré du nombre de ses utilisateurs.",
  ...
  render(ctx) {
    ...
    const toYL = n => gy1 - (gy1 - gy0) * n / maxN;          // linear: n
    const toYQ = n => gy1 - (gy1 - gy0) * (n * n) / (maxN * maxN); // quadratic: n²
    ...
    <text x="${gx0}" y="${gy0 - 18}" text-anchor="middle"
          font-family="Ubuntu, sans-serif" font-size="12" fill="${p.muted}" opacity="0.35">valeur</text>`;
    ...
    for (const d of diagrams) {
      ...
      <text x="${cx}" y="${cy + r + 28}" text-anchor="middle"
            ...
            fill="${col}" opacity="${isLarge ? '0.72' : '0.45'}">n=${n} → ${links} liens</text>`;
    }
