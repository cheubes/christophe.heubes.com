module.exports = {
  ...
  tagline:     "La mémoire de travail ne retient que 7 ± 2 éléments à la fois.",
  description: "Principe formulé par George Miller en 1956 : la capacité de la mémoire de travail humaine est limitée à environ sept éléments, avec une variation de plus ou moins deux.",

  render(ctx) {
    const { palette: p } = ctx;

    const cy = 470;   // circle vertical center
    const r  = 42;    // radius for circles 1-7
    const spacing = 100;  // center-to-center
    const startX = 200;   // center of circle 1

    // ── 7 gold circles (within capacity) ─────────────────────────────────
    ...
    // ── Circle 8 — borderline (smaller, muted) ────────────────────────────
    ...
    // ── Circle 9 — overflow (smaller still, very faint) ───────────────────
    ...
    // ── Limit marker between circles 7 and 8 ─────────────────────────────
    const limitX = (startX + 6 * spacing + cx8) / 2;  // midpoint = 800+900/2 = 850
    const limit = `
    ...
    <text x="${limitX}" y="${cy - 72}" text-anchor="middle"
          ...
          letter-spacing="1.5" fill="${p.gold}" opacity="0.65">LIMITE</text>`;

    // ── Bracket under circles 1-7 ─────────────────────────────────────────
    const bracketX1 = startX - r - 4;   // 154
    const bracketX2 = startX + 6 * spacing + r + 4;  // 846
    const bracketY  = cy + r + 28;       // 542
    const bracket = `
